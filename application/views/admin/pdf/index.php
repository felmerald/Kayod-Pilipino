...
<head>
	...
	<title>Kayod_employee Admin Profile</title>
...
<body>
	...
	$query_admin_info = $this->db->query('SELECT first_name, last_name, middle_name, birthday, address, contact_number, phil_health_number, sss_number, gender, image, role, company_id, job_position_id, hired_date, status, email, position_title, password
									FROM kp_users
									LEFT JOIN  job_position 
									ON job_position.job_id = kp_users.job_position_id
									WHERE user_id ='.$this->session->userdata('login_id').'');
									foreach($query_admin_info->result() as $row): ?>
				<div class="col-sm-3">
